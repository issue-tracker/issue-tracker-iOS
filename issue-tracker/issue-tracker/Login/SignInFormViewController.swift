...
class SignInFormViewController: CommonProxyViewController, View {
    ...
    func bind(reactor: SignInFormReactor) {
        ...
        idTextField.rx.text.orEmpty.distinctUntilChanged().skip(1)
        ...
        passwordTextField.rx.text.orEmpty.distinctUntilChanged().skip(1)
            .map({Reactor.Action.checkPasswordTextField($0)})
            .bind(to: reactor.action)
            .disposed(by: disposeBag)
        
        passwordConfirmTextField.rx.text.orEmpty.distinctUntilChanged().skip(1)
            .map({Reactor.Action.checkPasswordConfirmTextField($0)})
            .bind(to: reactor.action)
            .disposed(by: disposeBag)
        
        emailTextField.rx.text.orEmpty.distinctUntilChanged().skip(1)
        ...
        nicknameTextField.rx.text.orEmpty.distinctUntilChanged().skip(1)
